...
# Now outputs the state variables and the robot's perception of the state, changed the implementation slightly as I
# defined the position in my previous model as [row, column], whereas in previous implementations I had done it the
# other way around since I assumed the positions gave the coordinates [x, y]. However in the way I had previously
# defined position, [row, column] corresponded to [y, x]
...
class Cuboid:
    ...
    def plot_cuboid(self, ax, alpha):
        ...
        ax.plot_surface(X_top + self.coords[0], Y_top + self.coords[1], Z_top, color=self.color, alpha=alpha, edgecolors='k')
...
def multiple_ray_cast(list_of_rays, objects_to_check_3d, camera):
    for ray in list_of_rays:
        ...
        if next_ray_cast:
            ...
            multiple_ray_cast(rays, objects_to_check_3d, camera)
...
#                     'Small positions': [tensor([1, 1])]}
# object_positions = {'Large positions': [tensor([3, 3]), tensor([1, 3])],
#                     'Medium positions': [tensor([1, 2]), tensor([2, 2])],
#                     'Small positions': [tensor([1, 1]), tensor([2, 1]), tensor([3, 2])]}
# object_positions = {'Large positions': [],
#                     'Medium positions': [tensor([1, 3]), tensor([2, 1]), tensor([2, 2]), tensor([2, 3])],
#                     'Small positions': [tensor([1, 2]), tensor([3, 1]), tensor([3, 2])],
#                     'Empty positions': [tensor([1, 1]), tensor([3, 3])]
#                     }
...
#                     }
# object_positions = {'Large positions': [tensor([3, 2]), tensor([3, 3])],
#                     'Medium positions': [tensor([2, 3]), tensor([2, 2]), tensor([1, 2])],
#                     'Small positions': [],
#                     'Empty positions': [tensor([1, 1]), tensor([1, 3]), tensor([2, 1]), tensor([3, 1])]
#                     }
...
multiple_ray_cast(list_of_rays, objects_to_check_3d, camera)
...
for cuboid in objects:
    print(f'{cuboid.size} in position {cuboid.pos} is {cuboid.visibility}')
for empty in empty_objects:
    print(f'{empty.size} in position {empty.pos} is {empty.visibility}')
