...
# Objects are cuboids, plots the cuboids in 3d grid, created function to iterate through every face of an object and
# return the nearest intersection or None if no intersection exists
...
def compute_plane_intersect(ray, face_planes):
...
def compute_plane_intersect1(ray, plane_dict):
...
def compute_face_intersect(ray, plane_dict):
    ndotu = plane_dict['normal'].dot(ray.dir)
    if abs(ndotu) < 1e-6:
        ...
        intersect = w + multiplier * ray.dir + plane_dict['point']

        if (intersect[0] >= plane_dict['xlims'][0] and intersect[0] <= plane_dict['xlims'][1]) and (intersect[1] >= plane_dict['ylims'][0] and intersect[1] <= plane_dict['ylims'][1]) and (intersect[2] >= plane_dict['zlims'][0] and intersect[2] <= plane_dict['zlims'][1]):
...
def ray_cast(ray, cuboid):
    smallest_multiplier = 1e6
    closest_intersect = None
    for plane_dict in cuboid.face_planes.values():
        si, intersect = compute_face_intersect(ray, plane_dict)  # Checks if ray intersects a face, returns None if no intersect
...
# t2, intersect = compute_face_intersect(list_of_rays[-2], objects[0].face_planes['side 4'])
t2, intersect = ray_cast(list_of_rays[-2], objects[1])  # cuboid input has no intersect then this throws up an error
