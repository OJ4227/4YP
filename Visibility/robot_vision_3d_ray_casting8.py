...
# return the nearest intersection or None if no intersection exists. Made the functions methods in the Ray class
...
class Ray:
    ...
    def compute_face_intersect(self, plane_dict):
        ...
        if abs(ndotu) < 1e-6:
            ...
            if (plane_dict['xlims'][0] <= intersect[0] <= plane_dict['xlims'][1]) and (
                    ...
                    plane_dict['zlims'][0] <= intersect[2] <= plane_dict['zlims'][1]):
                return multiplier, intersect
            else:
                return 1e7, None  # Multiplier output cannot be None because we use it later in a less than statement

    def ray_cast(self, cuboid):
        smallest_multiplier = 1e6
        closest_intersect = None
        for plane_dict in cuboid.face_planes.values():
            si, intersect = self.compute_face_intersect(plane_dict)  # Checks if ray intersects a face, returns None if no intersect
...
# Get a list of all the objects that aren't visible in 2D to check if they are in 3D
objects_to_check_3d = []
for cuboid in objects:
    if cuboid.visibility == 'not visible':
        objects_to_check_3d.append(cuboid)

t2, intersect = list_of_rays[-2].ray_cast(objects[1])
