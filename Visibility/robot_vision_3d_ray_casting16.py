...
# Created a way for the code to identify which empty grid positions are visibly empty
...
def check_empty_positions_in_specific_row(empties_in_row, list_of_objects, camera1):
    ...
    q1 = Point2(camera.pos[0], camera.pos[1])
...
# Load in objects
dims = [3, 3]
object_positions = generate_object_positions.model(dims)
...
#                     }
# object_positions = {'Large positions': [tensor([1, 3]), tensor([2, 2]), tensor([3, 3])],
#                     'Medium positions': [tensor([2, 3])],
#                     'Small positions': [tensor([3, 2])],
#                     'Empty positions': [tensor([1, 2]), tensor([1, 1])]
#                     }
