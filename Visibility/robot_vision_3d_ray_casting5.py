...
# Objects are cuboids, plots the cuboids in 3d grid, created method to calculate the plane equations of the faces of the
# cuboid
large = 3
medium = 2
small = 1
...
class Cuboid:
    ...
    def compute_plane_equations(self):
        ...
        self.face_planes[face] = {'type': 'top',
                                  'point': p1_top,
                                  'norm': normal_top,
                                  'xlims': xlims_top,
                                  'ylims': ylims_top,
        ...
        for segment in self.segments_2d:
            ...
            face = f'{p1}, {p2}'
            self.face_planes[face] = {'type': 'side',
                                      'point': p1,
                                      'norm': normal,
                                      'xlims': xlims,
                                      'ylims': ylims,
...
for cuboid in objects:
    for segment in cuboid.obstructing_segments:
        ...
        point1 = np.array([segment.source().x(), segment.source().y(), cuboid.height])
        point2 = np.array([segment.target().x(), segment.target().y(), cuboid.height])
...
#     p1_top = np.array([cuboid.segments_2d[0].source().x(), cuboid.segments_2d[0].source().y(), cuboid.height]).astype(float)
#     p2_top = np.array([cuboid.segments_2d[0].target().x(), cuboid.segments_2d[0].target().y(), cuboid.height]).astype(float)
#
#     normal_top = np.array([0, 0, 1])
#     face = f'{p1_top}, {p2_top}'
#
#     xlims_top = [p1_top[0], p1_top[0] + cuboid.len]
#     ylims_top = [min(p1_top[1], p2_top[1]), min(p1_top[1], p2_top[1]) + cuboid.len]
#     zlims_top = [cuboid.height, cuboid.height]
#     cuboid.face_planes[face] = {'type': 'top',
#                                 'point': p1_top,
#                                 'norm': normal_top,
#                                 'xlims': xlims_top,
#                                 'ylims': ylims_top,
#                                 'zlims': zlims_top}
#
#     for segment in cuboid.segments_2d:
#         p1 = np.array([segment.source().x(), segment.source().y(), 0]).astype(float)
#         p2 = np.array([segment.target().x(), segment.target().y(), 0]).astype(float)
#         p3 = np.array([segment.source().x(), segment.source().y(), objects[0].height]).astype(float)
#
#         normal = np.cross(p3 - p1, p2 - p1)
#         xlims = np.array([min(p1[0], p2[0]), max(p1[0], p2[0])])
#         ylims = np.array([min(p1[1], p2[1]), max(p1[1], p2[1])])
#         zlims = np.array([0, objects[0].height])
#
#         face = f'{p1}, {p2}'
#         cuboid.face_planes[face] = {'type': 'side',
#                                     'point': p1,
#                                     'norm': normal,
#                                     'xlims': xlims,
#                                     'ylims': ylims,
