...
# Objects are cuboids, plots the cuboids in 3d grid, implemented the full 3D ray casting for this specific problem
...
for ray in list_of_rays:
    for cuboid in objects_to_check_3d:
        multiplier, intercept = ray.ray_cast(cuboid)
        if multiplier is not None:
            ray.draw_ray(ax2, multiplier=multiplier)


# t2, intersect = list_of_rays[-2].ray_cast(objects[0])
...
# Plot the rays
# ray.draw_ray(ax2)
# for idx, ray in enumerate(list_of_rays):
#     ray.draw_ray(ax2)
