...
# Now it will only cast a ray to the nearest visible object, and it outputs the segment associated with the nearest face
# that it does intersect. Created a function that is essentially a wrapper for doing the ray_cast for a list of rays
...
class Ray:
    ...
    def ray_cast(self, list_of_objects, camera):
        ...
        return smallest_multiplier, closest_intersect, segment
    ...
    for cuboid in list_of_cuboids:
        for segment in cuboid.obstructing_segments:
            ...
            for point in ray_points:
                ...
                angle = calculate_angle_to_y_axis(normalized_direction[0:2]) ################################################# Need to fix this
                if abs(angle) <= camera.fov/2:
...
def multiple_ray_cast(list_of_rays, objects_to_check_3d, camera):
    for ray in list_of_rays:
        ...
        multiplier, intercept, segment = ray.ray_cast(objects_to_check_3d, camera)
        if multiplier is not None:
            ray.draw_ray(ax2, multiplier=multiplier)
        # if segment is not None:
        #     rays = create_rays_for_3d(, camera, 7)
        #     multiple
...
# object_positions = {'Large positions': [tensor([3, 3]), tensor([1, 3])], 'Medium positions': [tensor([2, 1]), tensor([2, 2])],
#                     'Small positions': [tensor([1, 1])]}
object_positions = {'Large positions': [tensor([3, 3]), tensor([1, 3])], 'Medium positions': [tensor([1, 2]), tensor([2, 2])],
                    'Small positions': [tensor([1, 1])]}
...
camera = Camera(np.array([1, -1, 0]), optic_range=7, fov=np.pi / 3)
...
list_of_rays = create_rays_for_3d(objects, camera, 7)
...
# for ray in list_of_rays:
#     # for cuboid in objects_to_check_3d:
#     multiplier, intercept = ray.ray_cast(objects_to_check_3d, camera)
#     if multiplier is not None:
#         ray.draw_ray(ax2, multiplier=multiplier)
...
for cuboid in objects:
    print(f'cuboid in position {cuboid.pos} is {cuboid.visibility}')
plt.show()
