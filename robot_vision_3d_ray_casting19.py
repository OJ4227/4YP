...
# State perception outputs now sample from a Delta distribution so that pyro can do inference, camera position is now
# sampled from a uniform discrete distribution
...
def compute_state_perception(state_variables, list_of_objects, list_of_empties):
    ...
    state_perception = {f"'{el}'": pyro.sample(f"'{el}'", dist.Delta(tensor(-1))).numpy().item() for el in
                        keys}  # Need to check this with Ricardo ###################################
    for cuboid in list_of_objects:
    ...
    for key, value in state_perception.items():
        if value == 'S':
            ...
            state_perception[key] = pyro.sample(f'{key}', dist.Delta(tensor(1))).numpy().item()
        elif value == 'M':
            # state_perception[key] = 2
            state_perception[key] = pyro.sample(f'{key}', dist.Delta(tensor(2))).numpy().item()
        elif value == 'L':
            # state_perception[key] = 3
            state_perception[key] = pyro.sample(f'{key}', dist.Delta(tensor(3))).numpy().item()
        elif value == 'Empty':
            # state_perception[key] = 0
            state_perception[key] = pyro.sample(f'{key}', dist.Delta(tensor(0))).numpy().item()
...
def model_3d(dims, camera_specs):
    ...
    num_camera_positions = 2 * dims[1] - 1
    camera_positions = np.linspace(1, dims[1], num_camera_positions)
    camera_x = pyro.sample('camera_x', Discrete_Uniform(camera_positions))
    camera_specs['pos'] = np.array([camera_x, camera_specs['y'], camera_specs['z']])
