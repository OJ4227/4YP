...
for dims in list_of_dims:
    for num in num_samples:
        ...
        for i in range(1, 11):
            r_file_path = f"C:/Users/order/Documents/Oxford/4th Year/4YP/Pyro practice/data/{dims[0]}x{dims[1]}/R results/results_{dims[0]}x{dims[1]}_{num}_samples/{dims[0]}x{dims[1]}_{num}_samples{i}/Version 2/"
        ...
        # Initialise a list to contain data frames which are the concatenation of the data frames for each value of i
        concat_alg_types1 = {}

        # Store the concatenations for each algorithm class
        # Ignore the 0th column as it doesn't contain useful information
        concat_alg_types1['Constraint'] = pd.concat(constraint_results[i] for i in range(10))

        concat_alg_types1['Score'] = pd.concat(score_results[i] for i in range(10))

        concat_alg_types1['Hybrid'] = pd.concat(hybrid_results[i] for i in range(10))

        concat_alg_types1['Rsmax2'] = pd.concat(rsmax2_results[i] for i in range(10))
        ...
        write_path = f'C:/Users/order/Documents/Oxford/4th Year/4YP/Pyro practice/data/{dims[0]}x{dims[1]}/Results/{dims[0]}x{dims[1]}_{num}_samples/Version 2/'
        constraint_averaged_results.to_csv(write_path + 'constraint_averaged_results.csv', index=False)
        score_averaged_results.to_csv(write_path + 'score_averaged_results.csv', index=False)
        hybrid_averaged_results.to_csv(write_path + 'hybrid_averaged_results.csv', index=False)
        rsmax2_averaged_results.to_csv(write_path + 'rsmax2_averaged_results.csv', index=False)
