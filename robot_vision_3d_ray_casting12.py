...
# Need to fix how it identifies which segments to cast rays over. Fixed the issue where if the viewing angle of the
# camera stops an entire segment from being seen then the code won't identify it as an obstructing segment
...
def calculate_angle_to_y_axis(unit_vector):
...
camera = Camera(np.array([1, -1, 0]), optic_range=5, fov=np.pi / 24)
